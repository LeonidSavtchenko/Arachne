...
%% Launch gamma simulator with specified parameters
    ...
    if ispc && ~remoteHPC
        ...
        command = sprintf('call Core\\scripts\\win-win\\run.bat %s', args1to5);
        if backgroundMode
            command = ['start ', command];
        end
    elseif ispc && remoteHPC
        % Run HPC kernel on remote cluster under Linux from this machine under Windows
        command = sprintf('call Core\\scripts\\win-lin\\run.bat %s "%s"', args1to5, hostParams);
    elseif isunix && ~remoteHPC
        % Run HPC kernel on this cluster under Linux
        command = sprintf('bash Core/scripts/lin-lin/run.sh %s "%s" %i', args1to5, hostParams, backgroundMode);
    elseif isunix && remoteHPC
