function GenerateCumulativeCppSrcFile(modFileNames_e, modFileNames_i, outDirPath)
    ...
    outFileLines{1, 1} = '#include <math.h> ';
    outFileLines{2, 1} = '';
    outFileLines{3, 1} = '#include "AllModCurrents.h"'; 
    outFileLines{4, 1} = '';
    
    for i = 1 : length(modFileNames_e)
        outFileLines{hdrSize + i, 1} = sprintf('#include "%s.cpp"', modFileNames_e{i});
    end

    outFileLines{i + 1, 1} = '';
    
    for j = 1 : length(modFileNames_i)
        outFileLines{i + 1 + j, 1} = sprintf('#include "%s.cpp"', modFileNames_i{j});
    end
